...
class Segmentation:
    ...
    def __init__(self, **keys):
        if 'segments' in keys: self._init2__(**keys)
        else: self._init1__(**keys)
    
    def _init1__(self,serie,order, activity, automatic=True, compute=True):
        self.serie=serie
        self.order=order
        ...
        # If there is a duplicate points, the variance computation will fail.
    ...
    def _init2__(self,serie,order,activity,sd_serie,breaking_points,segments,average_segment,dispersion_segment):
        self.serie=serie
        self.order=order
        self.activity=activity
    ...
    def get_all(self):
        return(self.serie,self.order,self.sd_serie,self.sd_serie,self.breaking_points,
